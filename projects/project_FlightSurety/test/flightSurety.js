...
contract('Flight Surety Tests', async (accounts) => {
    ...
    it('(airline) Primary contract account is the airline', async () => {
        let result = await config.flightSuretyData.isAirline.call(account[0]);
        // ASSERT
    ...
    it('(airline) Primary contract can change the operation status', async () => {
        // Ensure that access is denied for non-Contract Owner account
        let status = true;
        try {
            await config.flightSuretyData.setOperatingStatus(false, { from: config.testAddresses[2] });
    ...
    it('(airline) Until minimum 4 airlines need to be registered, airline status is approved', async () => {
        ...
        try {
            isApproved0 = config.flightSuretyData.isApproved('DEFAULT', { from: config.testAddresses[0] });
            await config.flightSuretyApp.registerAirline('airline1', { from: config.testAddresses[1] })
            isApproved1 = config.flightSuretyData.isApproved('airline1', { from: config.testAddresses[0] });
            await config.flightSuretyApp.registerAirline('airline2', { from: config.testAddresses[2] })
            isApproved2 = config.flightSuretyData.isApproved('airline1', { from: config.testAddresses[0] });
            await config.flightSuretyApp.registerAirline('airline3', { from: config.testAddresses[3] })
            isApproved3 = config.flightSuretyData.isApproved('airline1', { from: config.testAddresses[0] });
            await config.flightSuretyApp.registerAirline('airline4', { from: config.testAddresses[4] })
            isApproved4 = config.flightSuretyData.isApproved('airline1', { from: config.testAddresses[0] });
